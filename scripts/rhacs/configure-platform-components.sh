...
namespace_regex='^(demo-platform|demo-webhook|developer-devspaces|external-sender|openshift-devspaces|openshift-pipelines)$'

central_api() {
  local method=$1 path=$2 body=${3:-}
  local args=(-ksS -X "$method" -H "Authorization: Bearer $ROX_API_TOKEN" -H 'Content-Type: application/json')
  [ -n "$body" ] && args+=(--data "$body")
  curl "${args[@]}" "https://$ROX_ENDPOINT$path"
}

current=$(central_api GET /v1/config)
updated=$(jq -c --arg name "$rule_name" --arg regex "$namespace_regex" '
